...
class MailMerge(object):
    def __init__(self, file, remove_empty_tables=False):
        ...
        try:
            content_types = lxml.etree.parse(self.zip.open('[Content_Types].xml'))
            ...
            for part in self.parts.values():
                ...
                for parent in part.findall('.//{%(w)s}fldSimple/..' % NAMESPACES):
                    for idx, child in enumerate(parent):
                        ...
                        if m is None:
                            continue
                        parent[idx] = lxml.etree.Element('MergeField', name=m.group(1))
                ...
                for parent in part.findall('.//{%(w)s}instrText/../..' % NAMESPACES):
                    ...
                    for idx_begin, idx_end in fields:
                        ...
                        if m is None:
                            continue
                        parent[idx_begin] = lxml.etree.Element('MergeField', name=m.group(1))
    ...
    def __get_tree_of_file(self, file):
        ...
        return zi, lxml.etree.parse(self.zip.open(zi))
    ...
    def write(self, file):
        ...
        with ZipFile(file, 'w', ZIP_DEFLATED) as output:
            for zi in self.zip.filelist:
                if zi in self.parts:
                    xml = lxml.etree.tostring(self.parts[zi].getroot())
                    output.writestr(zi.filename, xml)
                elif zi == self._settings_info:
                    xml = lxml.etree.tostring(self.settings.getroot())
                    output.writestr(zi.filename, xml)
                else:
    ...
    def merge_templates(self, replacements, separator):
        ...
        for part in self.parts.values():
            ...
            if sepClass == 'section':
                ...
                newType = lxml.etree.SubElement(nextPageSec, '{%(w)s}type'  % NAMESPACES)
                            ...
                            if (j + 1) == lc:
                                if (i + 1) == lr:
                                    ...
                                    if sepClass == 'section':
                                        ...
                                        p   = lxml.etree.SubElement(child, '{%(w)s}p'  % NAMESPACES)
                                        pPr = lxml.etree.SubElement(p, '{%(w)s}pPr'  % NAMESPACES)
                                        pPr.append(intSection)
                                        parts.append(p)
                                    elif sepClass == 'break':
                                        pb   = lxml.etree.SubElement(child, '{%(w)s}p'  % NAMESPACES)
                                        r = lxml.etree.SubElement(pb, '{%(w)s}r'  % NAMESPACES)
                                        nbreak = lxml.etree.Element('{%(w)s}br' % NAMESPACES)
    ...
    def __merge_field(self, part, field, text):
        for mf in part.findall('.//MergeField[@name="%s"]' % field):
            ...
            for i, text_part in enumerate(text_parts):
                text_node = lxml.etree.Element('{%(w)s}t' % NAMESPACES)
                text_node.text = text_part
                nodes.append(text_node)

                # if not last node add new line node
                if i < (len(text_parts) - 1):
                    nodes.append(lxml.etree.Element('{%(w)s}br' % NAMESPACES))
